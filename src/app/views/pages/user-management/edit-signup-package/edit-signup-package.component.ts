...
@Component({
  selector: 'kt-edit-signup-package',
  templateUrl: './edit-signup-package.component.html',
  styleUrls: ['./edit-signup-package.component.scss']
})

export class EditSignupPackageComponent implements OnInit {
  ...
  user: User;
	...
	ngOnInit() {
		...
		const routeSubscription =  this.activatedRoute.params.subscribe(params => {
			...
			if (id) 
			{
			    ...
			    this.adminService.postData('signup_getPackageById',dict).subscribe((response: any) => {
	...
	initUser() 
	{
		...
		if (!this.user.id) {
			...
			this.subheaderService.setBreadcrumbs([
				...
				{ title: 'Signup Packages',  page: `user-management/signup-package` },
				{ title: 'Create Signup Package', page: `user-management/signup-package/add` }
			]);
			
			return;
		}
		this.subheaderService.setTitle('Edit Signup Package');
		this.subheaderService.setBreadcrumbs([
			// { title: 'Agent Management', page: `user-management` },
			{ title: 'Signup Packages',  page: `user-management/signup-package` },
			{ title: 'Edit Signup Package', page: `user-management/signup-package/edit`, queryParams: { id: this.user.id } }
		]);
	...
	goBackWithId()
	{
		const url = '/user-management/signup-package';
	...
	refreshUser(isNew: boolean = false, id = 0) {
		...
		url = `/user-management/signup-package/edit/${id}`;
		this.router.navigateByUrl(url, { relativeTo: this.activatedRoute });
	}
	...
	addUser(_user: User, withBack: boolean = true)
	{
		...
		let dict = {
      	...
      	this.adminService.postData('signup_create_package',dict).subscribe((response:any) => {
      		this.ngxService.stop();
      		if(response.status == 1)
      		{
      			const message = `New Signup Package has been added successfully.`;
	...
	updateUser(_user: User, withBack: boolean = false)
	{
		...
		let dict = {
      	...
      	this.adminService.postData('signup_updatePackage',dict).subscribe((response:any) => {
      		this.ngxService.stop();
      		if(response.status == 1)
      		{
				const message = 'Signup Package details updated successfully.';
